...
function API:Kick(steamid, reason)
	RunConsoleCommand('ba', 'kick', steamid, reason)
end

function API:Ban(steamid, minutes, reason)
	if sgs[steamid] then
		if (minutes ==  0) then
			RunConsoleCommand('ba', 'perma', steamid, 'Cheating Infraction: https://portal.superiorservers.co/cache/sgs/' .. sgs[steamid] .. '.jpg')
		else
			RunConsoleCommand('ba', 'ban', steamid, minutes .. 'mi', 'Cheating Infraction: https://portal.superiorservers.co/cache/sgs/' .. sgs[steamid] .. '.jpg')
		end
		sgs[steamid] = nil
	else
		if (minutes ==  0) then
			RunConsoleCommand('ba', 'perma', steamid, 'Cheating Infraction')
		else
			RunConsoleCommand('ba', 'ban', steamid, minutes .. 'mi', 'Cheating Infraction')
		end
	end
end
