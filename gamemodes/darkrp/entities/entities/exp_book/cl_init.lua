...
function ENT:Draw3D2D()
	...
	-- Stats
	drawStatBar('Чернила: ' .. self:GetInk() .. '/' .. self:GetMaxInk(), o.y + 85, self:GetInk() / self:GetMaxInk())
	drawStatBar('Здоровье: ' .. self:GetHP() .. '/'..self.MaxHP, o.y + 160, self:GetHP() / self.MaxHP)
	local prinrPerc = predict(self:GetLastPrint(), rp.cfg.PrintDelay)
	drawStatBar('Прогресс: ' .. math_Clamp(prinrPerc, 0, 1) * 100 .. '%', o.y + 235, prinrPerc)
	cam_End3D2D()
end