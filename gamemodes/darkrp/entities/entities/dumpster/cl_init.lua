...
function ENT:Draw()
	...
	if self:GetDTInt(0) - CurTime() > 0 then
		...
		cam.Start3D2D(pos + Vector(0, 0, 50), ang + Angle(0, 90, 90), .1)
			draw.SimpleTextOutlined("Мусорка", '3d2d', 0, 0, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
		cam.End3D2D()
	end
end
