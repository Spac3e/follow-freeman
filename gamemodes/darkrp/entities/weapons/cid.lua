...
SWEP.WorldModel = "models/dorado/tarjeta2.mdl"
SWEP.Spawnable = true
if CLIENT then
	...
	SWEP.IronSightsAng = Vector(1.1, -2, 0)

	SWEP.VElements = {
		["card"] = { type = "Model", model = "models/dorado/tarjeta2.mdl", bone = "ValveBiped.cube3", rel = "", pos = Vector(0.465, 0.5, -1.57), angle = Angle(0, -20, -160.094), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
	}

  SWEP.WElements = {
  	["card"] = { type = "Model", model = "models/dorado/tarjeta2.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(4.375, 2.128, -1.54), angle = Angle(171.22, 134.626, 0.542), size = Vector(1.098, 1.098, 1.098), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
  }
end
...
function SWEP:PrimaryAttack()
	...
	if (CLIENT) then
		if (!IsValid(ply) or !ply:Alive()) then return end
		RunConsoleCommand("say", "/me Показал ID Карту под номером: "..id..". Лояльность: "..rp.GetTerm('loyalty')[loyality]..". Статус: "..status)
	end
