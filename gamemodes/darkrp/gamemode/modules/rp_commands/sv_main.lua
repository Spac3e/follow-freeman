...
local function Whisper(pl, text, args)
	...
	rp.LocalChat(CHAT_LOCAL, pl, 90, colors.Red, '[Whisper] ', pl, text)
end
...
local function Yell(pl, text, args)
	...
	rp.LocalChat(CHAT_LOCAL, pl, 600, colors.Red, '[Yell] ', pl, text)
end
...
local function PlayerAdvertise(pl, text, args)
	...
	if pl:CanAfford(cost) then
		...
		rp.GlobalChat(CHAT_NONE, colors.Red, '[Advertisement] ', pl, text)
	else
