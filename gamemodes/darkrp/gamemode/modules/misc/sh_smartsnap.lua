...
local function SnapPress()
	if ConVar("snap_clickgrid") ~= 0 and not snapclick then
		snapclick = true
		snapclickfade = CurTime()
	elseif ConVar("snap_clickgrid") == 0 or snapclick then
		if (snaplock or snapcursor) then
			snaptime = false
		else
			local toggledelay = GetConVar("snap_toggledelay")
...
local function OnKeyPress(player, key)
	if (key == IN_USE and GetConVar("snap_disableuse") == 0) then
		SnapPress()
	end
end
...
local function OnThink()
	...
	if (GetConVar("snap_enabletoggle") ~= 0) then
		if (snapkey and snaptime and not snaplock) then
			if (CurTime() > snaptime + GetConVar("snap_lockdelay")) then
				snaplock = true
				snaptime = false
			end
		end
	...
	if (not target.locked and locked and GetConVar("snap_revertaim") ~= 0) then
		if (snapcursor) then
...
local function CalculateGridAxis(L)
	...
	local grid = math.Clamp(math.floor(length / (2 * GetConVar("snap_gridsize"))) * 2, 2, GetConVar("snap_gridlimit"))
	local offset = math.Clamp(GetConVar("snap_gridoffset") / length, 0, 1 / grid)
...
local function DrawGridLines(vOrigin, vSX, vSY, gridLines, offsetX, offsetY, sign)
	local centerline = (GetConVar("snap_centerline") ~= 0)
...
local function DrawGrid(vOrigin, vSX, vSY)
	...
	surface.SetDrawColor(0, 0, 0, math.Round(GetConVar("snap_gridalpha") * 255))
	DrawGridLines(vOrigin, vSX, vSY, LX.grid, LX.offset, LY.offset, 1)
	DrawGridLines(vOrigin, vSY, vSX, LY.grid, LY.offset, LX.offset, 1)
	surface.SetDrawColor(255, 255, 255, math.Round(GetConVar("snap_gridalpha") * 255))
	DrawGridLines(vOrigin, vSX, vSY, LX.grid, LX.offset, LY.offset, -1)
	DrawGridLines(vOrigin, vSY, vSX, LY.grid, LY.offset, LX.offset, -1)
end
...
local function OnPaintHUD()
	target.active = false
	if GetConVar("snap_clickgrid") ~= 0 and not snapclick then return end
	snapclickprev = snapclick
	snapclick = snapclickprev and snapclickfade > CurTime()
	if (GetConVar("snap_enabled") == 0) then return end
	...
	if (target.locked) then
		...
		if (class ~= 'prop_physics' and class ~= 'phys_magnet' and class ~= 'gmod_spawner' and GetConVar('snap_allentities') == 0) then return end
		if (GetConVar('snap_allentities') == 1) and class == "player" then return end
		...
		if (LocalPlayer():GetActiveWeapon():GetClass() ~= 'gmod_tool' and GetConVar('snap_alltools') == 0) then return end
		target.entity = entity
	end

	--ErrorNoHalt(collectgarbage("count"))
	if GetConVar("snap_gcboost") ~= 0 then
		collectgarbage("step", GetConVar("snap_gcstrength"))
	end
	...
	if (GetConVar("snap_boundingbox") ~= 0) then
	...
	if (GetConVar("snap_hidegrid") == 0) then
