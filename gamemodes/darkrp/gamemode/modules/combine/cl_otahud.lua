...
hook.Add( "PostDrawTranslucentRenderables", "rp.HUD.OTA", function()
    ...
    for _, ply in ipairs( RenderedAndSortedPlayers ) do
		...
		fact = rp.Factions[ply:GetDisguiseJobTable() and ply:GetDisguiseJobTable().faction or ply:GetFaction()];
        ...
        cam.Start3D2D( ply:GetPos(), Angle(0,yaw + 90,90), scale_3d2d );
            surface.SetDrawColor( rp.Factions[ply:GetDisguiseJobTable() and ply:GetDisguiseJobTable().faction or ply:GetFaction()].OTARelations or Color(150, 170, 200, 255) );
            surface.DrawRect(
            ...
            if (LocalPlayer():IsLineOfSightClear(ply)) then
                ...
                local _TEMP = (ply:GetNetVar("FakedLoyalty") and rp.GetTerm('loyalty')[ply:GetNetVar("FakedLoyalty")] or JobTable.loyalty and rp.GetTerm('loyalty')[JobTable.loyalty]) or 'Неизвестно';
                cam.IgnoreZ( true );
