...
local function createChatsMenu(switch)
	...
	fr.newPM = ui.Create('DButton', function(self)
		...
		self:SetText("Новое личное сообщение")
	...
	fr.CreateThread = function(fr, chatType, id, title)
		...
		ui.Create('ui_listview', function(chat)
			...
			btn.DoRightClick = function(s)
				...
				m:AddOption("Удалить обсуждение", function()
					chat:Remove()
					btn:Remove()
					fr.Threads[id] = nil
	...
	fr:CreateThread(CHAT_LOCAL, 'local', 'Локальный')
	fr:CreateThread(CHAT_OOC, 'ooc', 'Общий')
	//fr:CreateThread(CHAT_CROSSSERVER, 'global', 'Глобальный')
...
hook("ChatRoomMessage", "ChatRoom.ParseMessage", function(chatType, plFrom, plTo, text)
	...
	if (chatType == CHAT_LOCAL) then
		fr:AddMessage(chatType, 'local', 'Локальный', plFrom, text)
	elseif (chatType == CHAT_OOC) then
		fr:AddMessage(chatType, 'ooc', 'Общий', plFrom, text)
	...
	elseif (chatType == CHAT_CROSSSERVER) then
		fr:AddMessage(chatType, 'global', 'Глобальный', plFrom, text:sub(3, #text))
		*/
	...
	elseif (chatType == CHAT_RADIO) then
		fr:AddMessage(chatType, 'radio', "Радио", plFrom, text)
	end
end)