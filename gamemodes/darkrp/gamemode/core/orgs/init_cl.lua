				...
				if (IsValid(fr.overRankNew)) then
					...
					local btnSubmit = ui.Create('DButton', function(btn)
						...
						btn.DoClick = function(s)
							...
							rp.RunCommand('orgrank', name, tostring(weight), invite and '1' or '0', kick and '1' or '0', canrank and '1' or '0', motd and '1' or '0')

							if (#orgranks < (LocalPlayer():HasUpgrade('org_prem') and 10 or 5)) then
				...
				if (IsValid(fr.overRankEdit)) then
					if (not ignore) then
						...
						if (invite ~= rank.Invite or kick ~= rank.Kick or canrank ~= rank.Kick or motd ~= rank.MoTD) then
							rp.RunCommand('orgrank', rank.Name, tostring(rank.Weight), invite and '1' or '0', kick and '1' or '0', canrank and '1' or '0', motd and '1' or '0')
							rank.Invite = invite
							rank.Kick = kick
							rank.Rank = canrank
							rank.MoTD = motd
...
function rp.orgs.LoadBanner(orgName)
	...
	wmat.Create('OrgBanner.' .. orgName, {
		URL = 'http://portal.superiorservers.co/server_scripts/rp/banners.php?org=' .. orgName,
		W = 128,
		H = 128
	}, function(material)
