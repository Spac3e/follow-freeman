...
local function DoLottery(ply, amount)
	...
	table.foreach(player.GetAll(), function(k, v)
		if v ~= ply then
			GAMEMODE.ques:Create("Лотерея! Для учасния нужно " .. rp.FormatMoney(LotteryAmount) .. " вы готовы?", "lottery"..tostring(k), v, 30, EnterLottery, ply, v)
		end
	end)
...
hook("PlayerDeath","DemoteOnDeath",function(v, k)
	if (v:Team() == TEAM_MAYOR) then
		...
		rp.FlashNotifyAll("Государство", rp.Term('MayorHasDied'))
	end
end)
